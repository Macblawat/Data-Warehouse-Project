...
JOIN warehouse_imp.dbo.Department as t3 ON t2.DepartmentName=t3.DepartmentName
...
MERGE INTO Lecturer as TT
	...
	WHEN MATCHED AND (
     ST.SalaryRange <> TT.SalaryRange
    OR ST.Title <> TT.Title
    OR ST.AgeCategory <> TT.AgeCategory
    OR ST.DepartmentID <> TT.DepartmentID
) THEN
    UPDATE SET
        TT.IsCurrent = 0
